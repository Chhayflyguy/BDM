<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h2 class="font-bold text-2xl text-gray-900 leading-tight">{{ __('messages.manage_products') }}</h2>
                <p class="text-sm text-gray-600 mt-1">{{ __('messages.manage_retail_products_catalog') }}</p>
            </div>
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-xl shadow-md overflow-hidden border border-gray-100">
            <div class="px-6 py-4" style="background: linear-gradient(to right, rgb(61, 109, 222), rgb(52, 95, 210));">
                <h3 class="text-lg font-semibold text-white">{{ __('messages.products_list') }}</h3>
            </div>
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead style="background: linear-gradient(to right, rgba(61, 109, 222, 0.1), rgba(61, 109, 222, 0.15));">
                        <tr>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">{{ __('messages.product_name') }}</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">{{ __('messages.price') }}</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">{{ __('messages.stock_quantity') }}</th>
                        ...
                        <tr class="transition-colors duration-150" style="--hover-bg: rgba(61, 109, 222, 0.05);" onmouseover="this.style.backgroundColor='rgba(61, 109, 222, 0.05)';" onmouseout="this.style.backgroundColor='';">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-3">
                                    ...
                                    <form action="{{ route('admin.products.destroy', $product) }}" method="POST" class="inline-block" onsubmit="return confirm({{ json_encode(__('messages.are_you_sure_delete_product')) }});">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-gray-500 text-lg font-medium mb-2">{{ __('messages.no_products_found') }}</p>
                                    <p class="text-gray-400 text-sm mb-4">{{ __('messages.get_started_create_first_product') }}</p>
