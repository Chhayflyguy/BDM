<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('messages.daily_expenses') }}</h2>
            <a href="{{ route('daily_expenses.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700">{{ __('messages.add_new_expense') }}</a>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        ...
        <div class="bg-white p-6 rounded-lg shadow-sm mb-8">
            ...
            <div class="bg-white p-6 rounded-lg shadow-sm mb-8 text-center">
                <h3 class="text-lg font-medium text-gray-500">{{ __('messages.total_expenses_for') }} {{ date('F', mktime(0,0,0,$currentMonth, 1)) }} {{ $currentYear }}</h3>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mt-8">
                <div class="p-6 text-gray-900 text-center">{{ __('messages.no_expenses_recorded_for_this_month') }}</div>
            </div>
            @endforelse
        </div>
    </div>
